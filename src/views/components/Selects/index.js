      ...
      <Controller
        render={({
          ...
          <div className={classnames("mb-3", { "position-relative": tooltip })} >
            ...
            <div className={classnames({ "input-group-merge": icon, "is-invalid": error })}>
              {icon ? <InputGroupText className={classnames({ "text-danger": error })}>{icon}</InputGroupText> : null}
              <Select
                {...rest}
                {...field}
                ...
                options={options}
                loadingMessage={() => <Spinner color="primary" size="md" type="grow" />}
                // menuIsOpen={true}
                onChange={(option) => {
                  field.onChange(option)
                  if (onChangeEvent) onChangeEvent(option ? option.value : null)
                }}
              />
            </div>
            {error && (<FormFeedback tooltip={tooltip ?? false}>{error.message}</FormFeedback>)}
          </div>
        )}
        name={name}
        control={control}
